...
//        }
      ...
      if viewStore.model.isDownloaded == false {
        if viewStore.model.modelType == .small {
          viewStore.send(.downloadModelTapped)
        }
      } else {
        stateSettings.isDownloading = false
      }
    }.onChange(of: viewStore.model.isDownloaded ) { _ in
      stateSettings.isDownloading = false
    }
    ...
    .contextMenu(viewStore.model.isDownloaded && viewStore.model.modelType != .small ? contextMenuBuilder() : nil)
    .enableInjection()
  }
